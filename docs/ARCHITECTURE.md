...
- `FJokateSourceControlProvider` — `Init` 이 `GET /api/ping` 으로 `root` 가 이 프로젝트인지 확인해 `bAvailable` 을 정한다. `Execute` 는 이름(`GetName`)으로 분기한다: `Connect`(ping + 전체 상태), `UpdateStatus`(`POST /api/states`, `ShouldUpdateHistory()` 면 파일마다 `GET /api/history`), `CheckIn`(`POST /api/confirm` `{message, only, editor_managed:true}`, 60초, 성공 시 `SetSuccessMessage` + 대상 Clean), `Revert`(`POST /api/discard` `{assets, editor_managed:true}`; 409 면 `error`·`locked` 를 `AddErrorMessge`; 언리얼이 패키지 언로드·리로드를 직접 하므로 브릿지는 안 부른다), `Delete`(읽기 전용 해제 후 `IFileManager::Delete` → 상태 갱신, 추적 안 하던 파일은 캐시에서 제거), `MarkForAdd`·`Copy`(데몬 호출 없이 상태만 갱신). 나머지(`Sync`·`CheckOut`·`Resolve` …)는 `CanExecuteOperation` 이 false 를 돌려주고 `Execute` 는 '로컬 전용이라 지원하지 않습니다' + `Failed`. 체크아웃·체인지리스트는 쓰지 않는다(`UsesCheckout/UsesChangelists = false`, `UsesFileRevisions = true`, `AllowsDiffAgainstDepot = true`)
- `FJokateSourceControlState` — 절대경로 + `EJokateFileState`(Unknown/Clean/Modified/Added/Deleted/Untracked/Missing). `CanEdit` 은 항상 true(잠금 없음), `CanCheckIn/CanRevert` 는 Modified·Added·Deleted(Added 의 Revert 는 파일 삭제라 툴팁에 명시). `History` 는 최신순 `FJokateSourceControlRevision` 배열이고 `GetCurrentRevision` 은 그 첫 항목
- `FJokateSourceControlRevision` — 확정 버전 하나(`SnapshotId`·`RevisionNumber`·`Sha`·`Description`·`Date`·`Action`·`FileSize`). `Get()` 은 `POST /api/extract {rel, sha}` 가 준 경로를 그대로 쓰거나(빈 목적지) 그 경로로 복사한다 → 내장 diff·이전 버전 열기. `GetRevision()` 은 `#<스냅샷 id>`, `GetUserName()` 은 `local`, 주석(annotate)·브랜치 소스는 없음
- 자동 재연결 — `Tick` 은 한 번이라도 붙은 뒤(`bWasConnected`) 끊기면 `TryReconnect` 를 돈다. 워커 스레드에서 `GET /api/ping` 을 조용히(`bQuiet`, 실패 로그는 Verbose) 보내고 게임 스레드로 돌아와 반영한다. 간격은 5 → 10 → 30초(최대)로 늘리고 성공하면 5초로 되돌리며, 끊긴 순간에만 Warning 한 줄을 남긴다. 성공하면 ping 의 `head_fix` 로 마지막 확정 표시를 갱신한다. 시도할 때마다 `daemon.json` 을 다시 읽으므로 데몬이 다른 포트로 떠도 붙는다
- 에디터 메뉴 — 모듈이 `UToolMenus::RegisterStartupCallback` 으로 `LevelEditor.MainMenu.Tools` 에 섹션 `JokateSourceControl`('Jokate 리비전 컨트롤')을 추가한다(파이썬 브릿지의 Tools > Jokate 와 별개라 브릿지 없이도 쓴다): '타임라인 열기'(`FPlatformProcess::LaunchURL`), '상태 새로 고침'(프로바이더가 Jokate 일 때 전체 `UpdateStatus` 비동기). 해제는 `ShutdownModule` 에서 `UnRegisterStartupCallback` + `UnregisterOwner`
- `editor_managed` — 플러그인이 보내는 `confirm`·`discard` 에는 `editor_managed:true` 가 붙어 데몬이 브릿지에 사이드카(의미 diff) 요청을 건너뛴다. 게임 스레드가 HTTP 응답을 기다리는 동안 브릿지 틱은 돌 수 없으므로, 요청하면 브릿지 시간 초과까지 양쪽이 서로 멈춘다
- `daemon.json` 정리 — 데몬은 종료할 때 파일 안의 `pid` 가 자기 자신일 때만 `daemon.json` 을 지운다. 새 데몬이 이미 덮어썼다면 그 파일은 새 데몬의 것이고, 지우면 플러그인이 포트를 못 찾는다
